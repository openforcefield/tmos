...
def get_target_atom(index, filtered_bonds, path_atoms):
    ...
    if len(bonded_atoms_charges) == 0:  # all the neighbors are happy, let it be
        ...
        bonded_atoms_charges = [
            (a, x)
            for a, x in bonded_atoms_charges
            if a.GetIdx() in path_atoms and get_atom_charge(a) != 0
        ]
        b_atom_ind = bonded_atoms_charges[0][
            0
        ].GetIdx()  # just take the first and maybe revise later
...
def get_ligand_attributes(
    ligand_mol, constant_atoms=None, resolved_atoms=None, verbose=False
):
    ...
    if not new_code:
        total_charge_after, hanging_bonds_after, charged_atoms_after = assess_atoms(
            ligand_mol
        )
        if verbose:
            print(
                f"Total ligand charge: {total_charge_after}, N_Hanging Bonds {hanging_bonds_after}"
            )
    ...
    if constant_atoms is None:
        constant_atoms = list(
            set(charged_atoms_before.keys()) & set(charged_atoms_after.keys())
        )
    unresolved_atoms = [
        x for x in charged_atoms_after.keys() if x not in charged_atoms_before
    ]
    if not unresolved_atoms and constant_atoms is not None:
        unresolved_atoms = [
            x for x in charged_atoms_after.keys() if x not in constant_atoms
        ]
    resolved_atoms = resolved_atoms if resolved_atoms else []
    resolved_atoms.extend(
        [x for x in charged_atoms_before.keys() if x not in charged_atoms_after]
    )

    if (
        not resolved_atoms
    ):  # If no atoms were resolved, then prefer the existing structure
        total_charge_before, hanging_bonds_before, charged_atoms_before = assess_atoms(
            ligand_mol_before
        )
        if verbose:
            print(
                f"Total ligand charge: {total_charge_before}, N_Hanging Bonds {hanging_bonds_before}"
            )
            print("Charged atom info:")
        ...
        return (
            total_charge_before,
            hanging_bonds_before,
            charged_atoms_before,
            ligand_mol_before,
        )
    ...
    if len(unresolved_atoms) > 0:
        ...
        if not neighbors:
            raise ValueError(
                "It's expected that all unresolved atoms are connected to the resolved atoms."
        ...
        for index in unresolved_atoms:
            ...
            if (
                get_atom_charge(unresolved_atom) == 0
            ):  # could have been resolved with a previous atom
                continue
            ...
            path_atoms = list(
                set(  # Get atoms in the path from atoms[index] to one of the constant_atoms
                    y
                    for x in constant_atoms
            ...
            if len(filtered_bonds) == 1:
                ...
                len(filtered_bonds) == 0
            ):  # No filtered_bonds so all bonds are aromatic or single. Delete a single bond
                filtered_bonds = [b for b, bo in zip(bonds, bond_orders) if bo == 1]
                if (
                    len(filtered_bonds) == 0
                ):  # If all bonds are aromatic, and assume it's right
                    continue

                b_atom_ind = get_target_atom(index, filtered_bonds, path_atoms)
                if b_atom_ind is not None:
                    bond_order = pt.GetDefaultValence(
                        unresolved_atom.GetAtomicNum()
                    ) - (sum([bo for bo in bond_orders]) - 1)  # for removed single bond
                ...
                if b_atom_ind is not None:
                    bond_order = pt.GetDefaultValence(
                        ...
                        [
                            bo
                            for b, bo in zip(bonds, bond_orders)
                            if b_atom_ind
                            not in [b.GetBeginAtomIdx(), b.GetEndAtomIdx()]
                        ]
                    )
        ...
        total_charge_after, hanging_bonds_after, charged_atoms_after, ligand_mol = (
            get_ligand_attributes(
                ligand_mol,
                constant_atoms=constant_atoms,
                resolved_atoms=unresolved_atoms,
                verbose=verbose,
            )
        )
...
def sanitize_complex(mol, overall_charge, verbose=False, value_missing_coord=0):
    ...
    for i, f in enumerate(frag_mols):
        ...
        for atom in atoms:
            ...
            total_charge, hanging_bonds, charged_atoms, m = get_ligand_attributes(
                m, verbose=verbose
            )
    ...
    tmc_mol = reform_metal_complex(
        (tm_mol, tm_ox), lig_info, coordinating_atoms, verbose=verbose
    )
    ...
    for i in coordinating_atoms_idx:
        ...
        bond_order = pt.GetDefaultValence(a.GetAtomicNum()) - sum(
            [
                bo
                for b, bo in zip(bonds, bond_orders)
                if tm_idx not in [b.GetBeginAtomIdx(), b.GetEndAtomIdx()]
            ]
        )
        if bond:
            ...
            if bond_order != old_bo:
                print(
                    f"Changing bond between {tm_idx} and {i} from {bond_type_dict[old_bo]}"
